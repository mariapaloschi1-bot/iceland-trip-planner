...
import { Calendar, MapPin, DollarSign, Info, AlertTriangle, Car, Package, Mountain, ExternalLink } from 'lucide-react';
import dynamic from 'next/dynamic';
import { itineraryData, budgetData, practicalInfo, experiencesDatabase, attractions, mapPoints } from '@/lib/data';
    ...
    <main className="min-h-screen bg-black text-ice-blue">
      <div className="relative bg-gradient-to-b from-black via-ice-purple-dark/20 to-black py-8 md:py-12 px-4 border-b-2 border-ice-blue/30">
        <div className="max-w-7xl mx-auto text-center">
          ...
          <p className="text-lg md:text-xl lg:text-2xl text-ice-light mb-2">16 agosto 18:50 arrivo • 23 agosto 08:20 partenza</p>
          <p className="text-sm md:text-base lg:text-lg text-ice-blue/80 max-w-4xl mx-auto px-4">
            Itinerario completo anti-vertigini: Reykjavík, Golden Circle, Costa Sud fino a Jökulsárlón e Snæfellsnes,
            con mappa interattiva e un marker per ogni luogo visitato.
          </p>
        </div>
      </div>
      ...
      <div className="sticky top-0 z-50 bg-black/95 backdrop-blur border-b-2 border-ice-blue/30">
        <div className="max-w-7xl mx-auto px-2 md:px-4">
          <div className="flex overflow-x-auto py-3 md:py-4 gap-2 no-scrollbar">
            <TabButton active={activeTab === 'itinerary'} onClick={() => setActiveTab('itinerary')} icon={<Calendar className="w-4 h-4 md:w-5 md:h-5" />}>
              Itinerario
            </TabButton>
            <TabButton active={activeTab === 'experiences'} onClick={() => setActiveTab('experiences')} icon={<Mountain className="w-4 h-4 md:w-5 md:h-5" />}>
              Esperienze
            </TabButton>
            <TabButton active={activeTab === 'budget'} onClick={() => setActiveTab('budget')} icon={<DollarSign className="w-4 h-4 md:w-5 md:h-5" />}>
              Budget
            </TabButton>
            <TabButton active={activeTab === 'info'} onClick={() => setActiveTab('info')} icon={<Info className="w-4 h-4 md:w-5 md:h-5" />}>
              Info
            </TabButton>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-3 md:px-4 lg:px-6 py-6 md:py-8">
        {activeTab === 'itinerary' && (
          <div className="space-y-6 md:space-y-8">
            <section className="mb-6 md:mb-8">
              ...
              <p className="text-sm md:text-base text-ice-blue/80 mb-4 max-w-4xl">
                Ogni puntino corrisponde a un luogo del viaggio. Cliccando su ogni marker vedi area/indirizzo,
                cos’è il posto e cosa ha di speciale. La mappa è filtrabile per giorno.
              </p>
        ...
        {activeTab === 'info' && (
          <div className="space-y-6 md:space-y-8">
            ...
            <InfoSection icon={<AlertTriangle className="w-5 h-5 md:w-6 md:h-6" />} title={practicalInfo.safety.title} warning>
              <ul className="space-y-2">
            ...
            <InfoSection icon={<ExternalLink className="w-5 h-5 md:w-6 md:h-6" />} title={practicalInfo.references.title}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {practicalInfo.references.list.map((source, idx) => (
                  <a
                    key={idx}
                    href={source.url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="bg-black p-4 rounded-lg border border-ice-blue/30 hover:border-ice-purple/60 transition-all"
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="font-bold text-ice-light text-sm md:text-base">{source.name}</p>
                        <p className="text-xs text-ice-blue/70 mt-1 break-all">{source.url}</p>
                      </div>
                      <ExternalLink className="w-4 h-4 flex-shrink-0 mt-1 text-ice-purple-light" />
                    </div>
                  </a>
                ))}
              </div>
      ...
      <footer className="bg-gradient-to-t from-black via-ice-purple-dark/20 to-black border-t-2 border-ice-blue/30 mt-12 md:mt-16 py-6 md:py-8">
        <div className="max-w-7xl mx-auto px-4 text-center">
          <p className="text-ice-blue/70 text-sm md:text-base">
            🧊 Itinerario Islanda anti-vertigini • con mappa interattiva punto per punto
          </p>
          <p className="text-ice-blue/50 text-xs md:text-sm mt-2 max-w-3xl mx-auto">
            Se vuoi poi posso aggiungere anche una Google Maps con tutte le tappe, hotel consigliati, stima costi dettagliata o PDF finale.
          </p>
        </div>
      </footer>
    </main>
    ...
    <div className="bg-gradient-to-br from-gray-900 via-ice-purple-dark/10 to-black rounded-xl overflow-hidden border-2 border-ice-blue/30 hover:border-ice-purple/60 transition-all">
      ...
      <div className="p-4 md:p-6">
        ...
        <div className="space-y-1.5 md:space-y-2 text-xs md:text-sm">
          {experience.duration && <p className="text-ice-blue/70">⏱️ Durata: {experience.duration}</p>}
          {experience.booking && <p className="text-ice-blue/70">📝 {experience.booking}</p>}
          {experience.website && (
            <a
...
function InfoSection({ icon, title, children, warning = false }: any) {
  return (
    <div className={`rounded-xl p-4 md:p-6 border-2 ${warning ? 'bg-gradient-to-br from-red-950/20 to-black border-red-500/40' : 'bg-gradient-to-br from-gray-900 to-black border-ice-blue/30'}`}>
      <div className="flex items-center gap-2 mb-3 md:mb-4">
        <div className={`${warning ? 'text-red-300' : 'text-ice-purple-light'}`}>{icon}</div>
        <h3 className="text-xl md:text-2xl font-bold text-ice-light">{title}</h3>
      </div>
      {children}
    </div>
  );
